...
import { StepBar, Step1, Step2, Step3, Step4, Step5 } from "./Formsteps ";
...
// ─── JWT helper: decode payload from refresh token ────────────────────────────
function getUserIdFromRefreshToken(): string {
  try {
    const token =
...
export const NewShipmentTab = () => {
  ...
  useEffect(() => {
    if (userId) {
      setForm(p => ({ ...p, sender_id: userId, created_by: userId }));
    }
  }, [userId]);
  ...
  const setBulk = useCallback((updates: Partial<ShipmentForm>) => {
    setForm(p => ({ ...p, ...updates }));
    setErrors([]);
  }, []);
  ...
  const handleMapClick = useCallback((lat: number, lng: number) => {
    setBulk({
      delivery_lat: parseFloat(lat.toFixed(6)),
      delivery_lng: parseFloat(lng.toFixed(6)),
    });
  }, [setBulk]);
  ...
  return (
    ...
    >
      ...
      <div className="bg-white p-6 border-r border-gray-100 flex flex-col h-full">
        {submitted ? (
          <Success id={trackingId} onReset={handleReset} />
        ) : (
          <>
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-100 shrink-0">
              ...
              {step < 5 && (
                <button
                  ...
                  className="px-6 py-2 bg-orange-500 text-white text-sm font-semibold rounded-lg shadow-sm shadow-orange-200 flex items-center gap-2 hover:bg-orange-600 transition"
                >
                  Continue →
                </button>
              )}
...
export const ShipmentsTab = () => {
  ...
  const clearFilters = () => {
    setFilterDate(""); setFilterCity(""); setFilterSender(""); setFilterPhone("");
  };
    ...
    <>
      {selected && !mapShipment && (
        <ShipmentDetailModal
          ...
          onViewMap={() => setMapShipment(selected)}
        />
      )}
      {mapShipment && mapShipment.delivery_lat !== null && (
        <MapModal
          lat={mapShipment.delivery_lat!}
          lng={mapShipment.delivery_lng!}
          title={`${mapShipment.tracking_id} — ${mapShipment.receiver_city}`}
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="ml-auto text-[11px] text-red-400 hover:text-red-500 font-semibold transition">
                Clear all
              </button>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            <div
              className="grid gap-0 bg-gray-50 border-b border-gray-100 px-4 py-2.5"
